...
def apply_optional_filters(row: pd.Series, prev_row: pd.Series | None, cfg: OptionalFilterConfig) -> FilterResult:
    ...
    _apply("trend_sma50", cfg.use_trend_filter,
           pd.notna(row.get("SMA50")) and row["Close"] > row["SMA50"])
    _apply("long_trend_sma200", cfg.use_long_trend_filter,
           pd.notna(row.get("SMA200")) and row["Close"] > row["SMA200"])
    _apply("volume_confirmation", cfg.use_volume_filter,
    ...
    _apply("atr_normalized_overshoot", cfg.use_atr_filter,
           pd.notna(row.get("BB_Overshoot_ATR")) and row["BB_Overshoot_ATR"] <= cfg.atr_overshoot_max)
    _apply("candle_quality_clv", cfg.use_candle_quality_filter,
           pd.notna(row.get("Close_Location_Value")) and row["Close_Location_Value"] >= cfg.candle_quality_min_clv)
    _apply("relative_strength", cfg.use_relative_strength_filter,
           pd.notna(row.get("RS_20D")) and row["RS_20D"] > 0)
    _apply("market_regime_bull", cfg.use_market_regime_filter,
           row.get("Market_Regime") == "BULL")
    _apply("fresh_breakout_only", cfg.use_fresh_breakout_only,
           row.get("Breakout_Type") == FRESH_BREAKOUT)
