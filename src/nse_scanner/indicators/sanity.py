...
def check_bollinger_ordering(bb_upper: float, bb_middle: float, bb_lower: float) -> None:
    ...
    if not (bb_upper >= bb_middle >= bb_lower):
        raise SignalMathError(f"Bollinger ordering violated: upper={bb_upper} middle={bb_middle} lower={bb_lower}")


def check_heikin_ashi_ordering(
    ha_high: float, ha_low: float, ha_open: float, ha_close: float, tol: float = 1e-9
) -> None:
