...
def main(argv: list[str] | None = None) -> int:
    ...
    now_ist = (
        datetime.now(IST)
        if not args.as_of
        else datetime.combine(datetime.fromisoformat(args.as_of).date(), datetime.min.time(), tzinfo=IST).replace(
            hour=16
        )
    )
    ...
    try:
        holidays = resolve_holidays_for_run(
            "config/nse_holidays.yaml", now_ist.date(), strict=not args.allow_missing_holidays
        )
    ...
    print(
        f"Signals: {result.signals_current} current, {result.signals_stale} stale. "
        f"RUN_HEALTH={result.run_health}. Price basis: {result.price_basis}. Report: {report_path}"
    )
    if result.needs_bootstrap:
        print(
            f"NOTICE: most of the universe lacks sufficient history — run "
